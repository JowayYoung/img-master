...
async function groupBy(mode) {
	const result = await utils.mapFile(
		process.cwd() + "/src",
		filePath => outputRes(filePath, mode)
	);
...
async function addUp() {
	...
	for (let v of imgList) {
		...
		Object.assign(count, { [v]: files.length });
	}
